...
2.2 Оберіть, як отримати код підтвердження — СМС або дзвінок.

2.3 Введіть **SMS-код** або код з дзвінка, отриманий на ваш номер:

![sms_code](./img/comfy/sms_code.png)

> Для того, щоб не вводити номер/код кожен раз — можна поставити чекбокс **Запам'ятати мене**.

2.4 Натисніть **Увійти** для входу в акаунт (особистий кабінет).

3.1 Для входу через Google натисніть на відповідну кнопку, увійдіть або оберіть свій акаунт, вкажіть номер телефону для зв'язку з акаунтом, та натиснить **Увійти**.
